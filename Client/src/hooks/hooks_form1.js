...
const HooksForm1 = () => {
	...
	const [state, dispatch] = useReducer(FormReducer.FormReducer, FormReducer.initState)
	...
	const handleuseReducerEdit = (event) => (
		dispatch(ACTIONS.user_input_edit(event.target.value))
	);
	...
	return(
		...
		<form onSubmit={handleuseReducerSubmit}>
		    ...
		    <input id="useReducer" onChange={handleuseReducerEdit} type="text" />
		    <button type="submit"> Submit </button>
		</form>
		<br />
		<form onSubmit={context.useContextSubmit}>
		    <label> React useContext: </label>
		    <input id="useContext" onChange={context.useContextChange} type="type" />
		    <button type="submit"> Submit </button>
		</form>
		<br />
		...
		<p>Change: {state.usetext_Edit}</p>
		<p>Submit: {state.usetext_Submit}</p>
